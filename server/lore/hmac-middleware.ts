/**
 * Lore → MIP HMAC-SHA256 서명 검증 미들웨어
 * WO-MIP-2026-003 §2 인증 및 보안
 */
 ...
 */
export function verifyLoreHmacSignature(
  serviceId: string,
  timestamp: string,
  bodyHash: string,
...
/**
 * Express 미들웨어: Lore → MIP 요청 HMAC 검증
 * - X-Service-ID: lore
 * - X-Timestamp: Unix timestamp (밀리초)
 * - X-Signature: HMAC-SHA256(secret, "lore:{timestamp}:{bodyHash}")
 * - Replay Attack 방지 (±5분)
 */
export function loreHmacMiddleware(
  req: Request,
  res: Response,
  next: NextFunction
): void {
  ...
  // HMAC 검증
  const valid = verifyLoreHmacSignature(
    serviceId,
    timestamp,
    bodyHash,
    signature,
    ENV.loreMipSharedSecret
  );
...
/**
 * MIP → Lore 발신용 HMAC 서명 생성
 */
export function generateLoreSignature(
  body: string,
  timestamp: string
): string {
  const bodyHash = crypto.createHash("sha256").update(body).digest("hex");
  return crypto
    .createHmac("sha256", ENV.mipLoreSharedSecret)
